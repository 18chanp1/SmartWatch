...
//    alt_up_video_dma_screen_fill(pixel_dma_dev, color_display_background, 0);
    ...
    draw_image(pixel_dma_dev, data, WIDTH, HEIGHT);
    printf("end drawing..\n");
...
//---------------wifi----------------------------------
void get_image_array(){
	struct alt_up_rs232_dev *rs232;
		rs232 = alt_up_rs232_open_dev("/dev/wifi_rs232");
		if (rs232 == NULL)
		  {alt_printf("Error: could not open wifi_RS232 UART\n");}
		else
		  {alt_printf("Opened RS232 UART device\n");}

	//	wifiCommand(6,rs232);
		alt_up_rs232_disable_read_interrupt(rs232);
		char cmd[128] = "get_image(\"206.87.222.118\",\"submitserial\",\"-123.231859\",\"49.264399\")\n";
//		char cmd[128] = "dofile(\"wifi.lua\")\n";
//		char cmd[128] = "dofile(\"test_connect.lua\")\n";
		for(int i=0; i < strlen(cmd); i++){
			alt_up_rs232_write_data(rs232, cmd[i]);
			alt_printf("%c",cmd[i]);
		}
...
//						str[pos++]=receive;
					...
					else if(receive == ' '&&save ==1){
						count++;
						if(count%1000 == 0) printf("%d\n",count);
					}
//					temp_char = receive;
					else if(save &&  receive != '\n'){
						str[pos++]=receive;
...
void draw_image(alt_up_video_dma_dev* screen, alt_u8 image[][3], alt_u16 width, alt_u16 height) {
	...
	for (alt_u16 y = 0; y*yStep < height; y++) {
		for (alt_u16 x = 0; x*xStep < width; x++) {
			...
			if (bitWidth == 1) {
				color = ALT_UP_VIDEO_RESAMPLE_RGB_24BIT_TO_8BIT(color);

			} else {
				color = ALT_UP_VIDEO_RESAMPLE_RGB_24BIT_TO_16BIT(color);
			}
