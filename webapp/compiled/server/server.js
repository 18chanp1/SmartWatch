...
app.post('/requestImage', async function (req, res) {
    ...
    if (!found) {
        ...
        for (var latlong of foundWorkOut.locationRecord) {
            let coord = [latlong.lng, latlong.lat];
            coor.push(coord);
        }
        coor.push(coor[0]);
        // console.log(coor);
        ...
        usersDb.read({ de1serial: incomingWorkout.de1Serial.toString() }).then((document) => {
            ...
            for (let workout of copyWorkouts) {
                if (workout.id == incomingWorkout.id) {
                    ...
                    usersDb.update({ de1serial: incomingWorkout.de1Serial.toString() }, { uploads: copyWorkouts }).then((result) => {
                        console.log("success");
        ...
        usersDb.read({ username: usr }).then((document) => {
            return new Promise((resolve, reject) => {
                if (document == null) {
                    ...
                    let workoutsArr = document.uploads;
                    let index = workoutsArr.findIndex(o => o.id == workoutID);
                    currSer = document.currentSerial - 1;
            ...
            return new Promise((resolve, reject) => {
                ...
                else if (updatedDoc.modifiedCount == 1) {
...
app.post("/changePerms", session.middleware, (req, res) => {
    try {
        ...
        let params = req.query;
        let workoutID = params.id;
        let changeToPublic = (params.changeToPublic === "true");
        let before;
    ...
    try {
        ...
        usersDb.read({ username: usr }).then((document) => {
            return new Promise((resolve, reject) => {
                ...
                let upload = document.uploads.find(o => o.id == workoutID);
                let workout = document.workouts.find(o => o.id == workoutID);
                if (upload == undefined && workout == undefined) {
                    resolve(document.uploads);
                }
                else {
...
app.get("/workouts", session.middleware, (req, res) => {
    try {
        let usr = req.username;
        usersDb.read({ username: usr }).then((document) => {
            let usrWorkouts = document.uploads;
...
app.get("/publicworkouts", session.middleware, (req, res) => {
    try {
        ...
        usersDb.read({ username: usr }).then((user) => {
            ...
            let publicWorkouts = user.uploads.filter(workout => workout.isPublic && workout.completed);
...
app.get("/stats", session.middleware, (req, res) => {
    try {
        ...
        usersDb.read({ username: usr }).then((document) => {
            ...
            LeaderboardObject = fitnessUser.computeStatistics(documents);
...
app.get("/uploads", session.middleware, (req, res) => {
    try {
        usersDb.read({ username: req.username }).then((user) => {
            if (user == null) {
                res.status(404).send("Could not find uploads");
                return;
            }
            let notDoneUploads = user.uploads.filter((workout) => { return !workout.completed; });
...
app.get("/serial", session.middleware, (req, res) => {
    try {
        usersDb.read({ username: req.username }).then((user) => {
            ...
            res.status(200).send(JSON.stringify(user.de1serial));
        }, (err) => {
...
app.post("/routeupload", session.middleware, async (req, res) => {
    try {
        ...
        usersDb.read({ username: req.username }).then(async (user) => {
            if (user == null) {
                res.status(404).send("Could not find workouts");
                return;
            }
            let workoutsArr = user.uploads;
            let serial = (await usersDb.read({ currentSerial: { $exists: true } })).currentSerial;
            (await usersDb.update({ currentSerial: { $exists: true } }, { currentSerial: serial + 1 }));
            workoutsArr.push({
            ...
            usersDb.update({ username: req.username }, { uploads: workoutsArr }).then((success) => {
    ...
    try {
        ...
        if (user == null) {
            ...
            usersDb.read({ username: user }).then((fitnessUser) => {
                ...
                let correctPassword = isCorrectPassword(req.body.password, fitnessUser.password);
                if (correctPassword) {
