...
/**
 * Type declaration for each session. 
 */
type session =
{
	"username": string,
...
class SessionManager {
	...
	CookieMaxAgeMs: number = 60000000;
	...
	middleware = (request, response, next) =>
	{
		...
		request.username = this.sessions.get(<string> cookieID)!.username;
 ...
 */
function isCorrectPassword (password: string, saltedHash: string): boolean{
 ...
 */
function createSaltedHash(password: string): string{
	//create salt
